...
public interface UserWorkingStatusExceptionRepository extends JpaRepository<UserWorkingStatusExceptionEntity, String> {
    ...
    @Query("FROM UserWorkingStatusExceptionEntity wse WHERE wse.userId = :userId AND (wse.startDate BETWEEN :rangeStartDate AND :rangeEndDate OR wse.endDate between :rangeStartDate AND :rangeEndDate OR (wse.startDate <= :rangeStartDate AND wse.endDate >= :rangeEndDate))")
    ...
    @Query("FROM UserWorkingStatusExceptionEntity wse WHERE wse.userId = :userId AND (:now BETWEEN wse.startDate AND wse.endDate OR wse.startDate = :now OR wse.endDate = :now)")
    ...
    @Query("FROM UserWorkingStatusExceptionEntity uwse WHERE uwse.userId IN (SELECT u.id FROM UserEntity u WHERE u.organizationId = :organizationId)")
